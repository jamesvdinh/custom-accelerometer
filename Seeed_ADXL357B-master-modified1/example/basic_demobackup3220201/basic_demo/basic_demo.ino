...
/* 2/14/20
    ...
    From page 4 on datahseet for the +-40g range, scale factor multiplier to raw data is 78 micro-g/LSB
    multiple scale factor with the raw data to get in unit of acceleratiuon of gravity g
    works with a hybrid interrupt and loop combination
    The DATA_READY signal from ADXL357 goes low when data is read
    On oscilloscope, can observe that the DATA_READY pulse width got shorter as data is read in loop()
    If no reading of data, the pulse width is 50% duty cycle
    Can't place read inside an interrupt since no response and code too deep into i2c
    ...
    after read accelerometer data then set pin 13 low to indicate completion of read data 
    tested to work at 125 Hz
    
    ****************input pin 2 interrupt signal from ADXL357 Data_Ready output
    ****************output pin 4 for oscillocscope monitor of reading ADXL357 and write to flash memory
...
// for ADXL357 stuff
//#define ABC // if define then do the calibration accroding to this Seed basic_demo code
#define CALI_BUF_LEN           15  // for 16 samples
#define CALI_INTERVAL_TIME     250 // this is for a delay
#define ODR                    0x06 // see page 38 table 44 for settings 
...
#define SCALE_FACTOR_40G        (78E-6) // 40g rnage scale factor is 78microg/LSB
...
/**************** ADXL357 setup stuff *********************/
  ...
  /********************interrupt ISR stuff******************/
  pinMode(4, OUTPUT);           // pin 4 for oscilloscope monitor
  delay(DELAY);
